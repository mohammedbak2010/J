-- H2N v6.0 - ULTIMATE FIX
-- دمج H2N v5.10 مع أفضل ميزات Zyphrot Hub
-- إصلاح كامل للزر الجانبي
...
task.spawn(function()
    ...
    lbl.Text = "This script will become paid after 3 days"
    ...
    lbl2.Text = "Join our Discord to buy it: discord.gg/wsUuRQYVB"
    ...
    lbl3.Text = "السكريبت سيصبح مدفوعاً بعد 3 أيام — انضم للديسكورد للشراء"
...
-- H2N Theme: Royal Gold × Obsidian — الذهب الملكي والليل الأسود
local T = {
    A   = Color3.fromRGB(255, 215, 100),  -- ذهب لامع
    B   = Color3.fromRGB(230, 185, 60),   -- ذهب داكن
    C   = Color3.fromRGB(200, 155, 30),   -- ذهب أعمق
    D   = Color3.fromRGB(255, 240, 180),  -- ذهب فاتح ناعم
...
-- ============================================================
-- SHIMMER: UIGradient دوار على كل Border في المينيو
-- ============================================================
...
-- TP DOWN (زر مباشر - ينقلك لأقرب أرض تحتك فوراً)
...
AP_ROUTES = {
    ...
    SHORT_LEND = Vector3.new(-483, -4, 95),
    SHORT_R1   = Vector3.new(-476, -6, 28),
    SHORT_REND = Vector3.new(-483, -4, 26),
    ShortEnabled = false,
}
...
local function startOptimizer()
    ...
    -- إيقاف أنيميشن الـ UI (تجميد اللون الذهبي)
    pcall(function()
...
-- ============================================================
-- SPEED COUNTER — عداد سرعة فوق اللاعب
-- ============================================================
task.spawn(function()
    local RunService = game:GetService("RunService")
...
pi = pi + 1
MakeToggle(protect, "DESYNC", pi, function(s) if sideButtonRefs["Desync"] then sideButtonRefs["Desync"](s, true) end end, function() return sideButtonVisibility["Desync"] == true end, "Desync")
pi = pi + 1
    ...
    if stateKey then
        sideButtonRefs[stateKey] = function(state, fromMenu)
            ...
            if not state and fromMenu then
                ...
                elseif stateKey == "Desync" and DesyncState.enabled then stopDesync()
                end
            end
            RefreshVisual()
        end
...
task.spawn(function()
    ...
    local _sideKeys = {"AutoPlayLeft","AutoPlayRight","SpeedBoost","FloatEnabled","AutoTPDown","DROP","Desync","AutoBat","TPDown"}
...
print("===============================================================")
print("H2N v6.0 - ULTIMATE FIX (Anti Lag Edition)")
print("===============================================================")