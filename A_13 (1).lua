...
local hubTitleLbl = New("TextLabel", {
    Text = "A_13", TextColor3 = Color3.fromRGB(100, 210, 255),
...
do
    ...
    if getgenv then
        if getgenv().A_13_AUTOLOAD == nil then
            getgenv().A_13_AUTOLOAD = false
        end
    end

    local autoLoadEnabled = (getgenv and getgenv().A_13_AUTOLOAD == true) or false
    local kickConn = nil

    if getgenv then
        if not getgenv().A_13_LAST_RELOAD then
            getgenv().A_13_LAST_RELOAD = 0
        end
    end
    ...
    local function doReload()
        ...
        local now = tick()
        if getgenv and (now - (getgenv().A_13_LAST_RELOAD or 0)) < 5 then return end
        if getgenv then getgenv().A_13_LAST_RELOAD = now end
    ...
    local function setAutoLoad(v)
        ...
        if getgenv then getgenv().A_13_AUTOLOAD = v end
