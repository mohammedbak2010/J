...
task.spawn(function()
    ...
    -- ====================== MOBILE BUTTONS (each button is independent & draggable) ======================
    local PURPLE_ON=Color3.fromRGB(200,0,25); local BLACK_OFF=Color3.fromRGB(8,8,8)

    -- helper: make a standalone draggable button frame
    local function makePBtn(l1, l2, defaultX, defaultY)
        local gui = Instance.new("ScreenGui", player:WaitForChild("PlayerGui"))
        gui.Name = "A13_Btn_"..l1..l2; gui.ResetOnSpawn=false; gui.IgnoreGuiInset=true; gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; gui.DisplayOrder=9

        local frame = Instance.new("Frame", gui)
        frame.Name = "BtnFrame"; frame.Size = UDim2.new(0,58,0,52)
        frame.Position = UDim2.new(0, defaultX, 0, defaultY)
        frame.BackgroundColor3 = BLACK_OFF; frame.BackgroundTransparency=0; frame.BorderSizePixel=0; frame.Active=true; frame.ZIndex=20
        Instance.new("UICorner", frame).CornerRadius = UDim.new(0,12)
        local bs = Instance.new("UIStroke", frame); bs.Color=Color3.fromRGB(40,40,50); bs.Thickness=1.2; bs.Transparency=0.2

        local t1 = Instance.new("TextLabel", frame); t1.Size=UDim2.new(1,0,0.52,0); t1.Position=UDim2.new(0,0,0.06,0); t1.BackgroundTransparency=1; t1.Text=l1; t1.TextColor3=Color3.fromRGB(255,255,255); t1.Font=Enum.Font.GothamBlack; t1.TextSize=11; t1.TextXAlignment=Enum.TextXAlignment.Center; t1.ZIndex=22
        local t2 = Instance.new("TextLabel", frame); t2.Size=UDim2.new(1,0,0.44,0); t2.Position=UDim2.new(0,0,0.52,0); t2.BackgroundTransparency=1; t2.Text=l2; t2.TextColor3=Color3.fromRGB(255,255,255); t2.Font=Enum.Font.GothamBlack; t2.TextSize=11; t2.TextXAlignment=Enum.TextXAlignment.Center; t2.ZIndex=22

        local btn = Instance.new("TextButton", frame); btn.Size=UDim2.new(1,0,1,0); btn.Position=UDim2.new(0,0,0,0); btn.BackgroundTransparency=1; btn.Text=""; btn.ZIndex=23

        -- drag state per button (shared across closures)
        local drag = {active=false, startInput=nil, startFrameX=0, startFrameY=0, moved=false}

        frame.InputBegan:Connect(function(inp)
            if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
                drag.active=true; drag.moved=false
                drag.startInput=Vector2.new(inp.Position.X, inp.Position.Y)
                drag.startFrameX=frame.Position.X.Offset
                drag.startFrameY=frame.Position.Y.Offset
            end
        end)
        UserInputService.InputEnded:Connect(function(inp)
            if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
                drag.active=false; drag.startInput=nil
            end
        end)
        UserInputService.InputChanged:Connect(function(inp)
            if not drag.active or not drag.startInput then return end
            if inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch then
                local dx=inp.Position.X-drag.startInput.X
                local dy=inp.Position.Y-drag.startInput.Y
                if math.abs(dx)>3 or math.abs(dy)>3 then
                    drag.moved=true
                    frame.Position=UDim2.new(0, drag.startFrameX+dx, 0, drag.startFrameY+dy)
                end
            end
        end)

        -- block click from firing when user was dragging
        local origClick = nil
        btn.MouseButton1Click:Connect(function()
            if drag.moved then drag.moved=false; return end
        end)

        return btn, bs, frame, drag
    end

    local function setPH(frame,bs,isOn) frame.BackgroundColor3=isOn and PURPLE_ON or BLACK_OFF; bs.Color=isOn and Color3.fromRGB(255,20,50) or Color3.fromRGB(40,40,50); bs.Transparency=isOn and 0 or 0.2 end

    -- Layout:
    --   RIGHT side (3): AUTO L, AUTO R, PLAY L
    --   LEFT  side (3): PLAY R, DROP, TP DOWN
    --   RIGHT side below (2): LOCK, CARRY SPD
    local vp = workspace.CurrentCamera.ViewportSize
    local bW=58; local bH=52; local gap=8
    local rX = math.floor(vp.X) - bW - 10
    local lX = 10
    local rStartY = math.floor(vp.Y/2) - math.floor((bH+gap)*1.5)
    local lStartY = math.floor(vp.Y/2) - math.floor((bH+gap)*1.5)

    local btnAAL,bsAAL,fAAL,dAAL = makePBtn("AUTO","L",  rX, rStartY)
    local btnAAR,bsAAR,fAAR,dAAR = makePBtn("AUTO","R",  rX, rStartY+(bH+gap))
    local btnPL, bsPL, fPL, dPL  = makePBtn("PLAY","L",  rX, rStartY+(bH+gap)*2)
    local btnPR, bsPR, fPR, dPR  = makePBtn("PLAY","R",  lX, lStartY)
    local btnDR, bsDR, fDR, dDR  = makePBtn("DROP","",   lX, lStartY+(bH+gap))
    local btnTP, bsTP, fTP, dTP  = makePBtn("TP","DOWN", lX, lStartY+(bH+gap)*2)
    local btnLK, bsLK, fLK, dLK  = makePBtn("LOCK","",   rX, rStartY+(bH+gap)*3)
    local btnCS, bsCS, fCS, dCS  = makePBtn("CARRY","SPD", rX, rStartY+(bH+gap)*4)
    ...
    if pState.CS then setPH(fCS,bsCS,true) end

    local function mutualOff(keep)
        local map={AAL={fAAL,bsAAL},AAR={fAAR,bsAAR},PL={fPL,bsPL},PR={fPR,bsPR},LK={fLK,bsLK}}
    ...
    btnAAL.MouseButton1Click:Connect(function() if dAAL.moved then dAAL.moved=false; return end; local ns=not pState.AAL; if ns then mutualOff("AAL") end; pState.AAL=ns; setPH(fAAL,bsAAL,ns); mobBtnRefs["AUTO L"].state=ns; if toggleSetters["Auto Left"] then toggleSetters["Auto Left"](ns) end; if ns then autoLOn=true; startAutoL() else autoLOn=false; stopAutoL() end end)
    btnAAR.MouseButton1Click:Connect(function() if dAAR.moved then dAAR.moved=false; return end; local ns=not pState.AAR; if ns then mutualOff("AAR") end; pState.AAR=ns; setPH(fAAR,bsAAR,ns); mobBtnRefs["AUTO R"].state=ns; if toggleSetters["Auto Right"] then toggleSetters["Auto Right"](ns) end; if ns then autoROn=true; startAutoR() else autoROn=false; stopAutoR() end end)
    btnPL.MouseButton1Click:Connect(function() if dPL.moved then dPL.moved=false; return end; local ns=not pState.PL; if ns then mutualOff("PL") end; pState.PL=ns; setPH(fPL,bsPL,ns); mobBtnRefs["PLAY L"].state=ns; if toggleSetters["Play Left"] then toggleSetters["Play Left"](ns) end; if ns then aplOn=true; startAutoPlayLeft() else aplOn=false; stopAutoPlayLeft() end end)
    btnPR.MouseButton1Click:Connect(function() if dPR.moved then dPR.moved=false; return end; local ns=not pState.PR; if ns then mutualOff("PR") end; pState.PR=ns; setPH(fPR,bsPR,ns); mobBtnRefs["PLAY R"].state=ns; if toggleSetters["Play Right"] then toggleSetters["Play Right"](ns) end; if ns then aprOn=true; startAutoPlayRight() else aprOn=false; stopAutoPlayRight() end end)
    btnDR.MouseButton1Click:Connect(function() if dDR.moved then dDR.moved=false; return end; setPH(fDR,bsDR,true); task.delay(0.35,function() setPH(fDR,bsDR,false) end); task.spawn(doDrop) end)
    btnTP.MouseButton1Click:Connect(function() if dTP.moved then dTP.moved=false; return end; setPH(fTP,bsTP,true); task.delay(0.35,function() setPH(fTP,bsTP,false) end); task.spawn(doTPDown) end)
    btnLK.MouseButton1Click:Connect(function() if dLK.moved then dLK.moved=false; return end; local ns=not pState.LK; if ns then mutualOff("LK") end; pState.LK=ns; setPH(fLK,bsLK,ns); if toggleSetters["Lock"] then toggleSetters["Lock"](ns) end; if ns then startBatAimbot() else stopBatAimbot() end end)
    btnCS.MouseButton1Click:Connect(function() if dCS.moved then dCS.moved=false; return end; local ns=not pState.CS; pState.CS=ns; setPH(fCS,bsCS,ns); slowDownEnabled=ns; if toggleSetters["Carry SPD"] then toggleSetters["Carry SPD"](ns) end end)
    ...
    UserInputService.InputBegan:Connect(function(input,processed)
        ...
        if kc==Keybinds.AutoLeft then
            local ns=not pState.PL; if ns then mutualOff("PL") end; pState.PL=ns; setPH(fPL,bsPL,ns); if toggleSetters["Play Left"] then toggleSetters["Play Left"](ns) end; if ns then aplOn=true; startAutoPlayLeft() else aplOn=false; stopAutoPlayLeft() end
        elseif kc==Keybinds.AutoRight then
            local ns=not pState.PR; if ns then mutualOff("PR") end; pState.PR=ns; setPH(fPR,bsPR,ns); if toggleSetters["Play Right"] then toggleSetters["Play Right"](ns) end; if ns then aprOn=true; startAutoPlayRight() else aprOn=false; stopAutoPlayRight() end
        elseif kc==Keybinds.MobAutoL and Keybinds.MobAutoL~=Enum.KeyCode.Unknown then
            local ns=not pState.AAL; if ns then mutualOff("AAL") end; pState.AAL=ns; setPH(fAAL,bsAAL,ns); mobBtnRefs["AUTO L"].state=ns; if toggleSetters["Auto Left"] then toggleSetters["Auto Left"](ns) end; if ns then autoLOn=true; startAutoL() else autoLOn=false; stopAutoL() end
        elseif kc==Keybinds.MobAutoR and Keybinds.MobAutoR~=Enum.KeyCode.Unknown then
            local ns=not pState.AAR; if ns then mutualOff("AAR") end; pState.AAR=ns; setPH(fAAR,bsAAR,ns); mobBtnRefs["AUTO R"].state=ns; if toggleSetters["Auto Right"] then toggleSetters["Auto Right"](ns) end; if ns then autoROn=true; startAutoR() else autoROn=false; stopAutoR() end
        elseif kc==Keybinds.BatAimbot then
            local ns=not pState.LK; if ns then mutualOff("LK") end; pState.LK=ns; setPH(fLK,bsLK,ns); if toggleSetters["Lock"] then toggleSetters["Lock"](ns) end; if ns then startBatAimbot() else stopBatAimbot() end
        elseif kc==Keybinds.CarrySpd then
            local ns=not pState.CS; pState.CS=ns; setPH(fCS,bsCS,ns); slowDownEnabled=ns; if toggleSetters["Carry SPD"] then toggleSetters["Carry SPD"](ns) end
