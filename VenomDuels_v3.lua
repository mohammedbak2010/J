...
do
    ...
    task.spawn(function()
        local spd = 1.5
        while task.wait() do
            _g.Rotation = (_g.Rotation + spd) % 360
        end
    end)
end
...
do
    ...
    task.spawn(function()
        local spd = 1.2
        while task.wait() do
            _tg.Rotation = (_tg.Rotation + spd) % 360
        end
    end)
end
...
do
    local _hg = Instance.new("UIGradient")
    ...
    task.spawn(function()
        local spd = 1.5
        while task.wait() do
            _hg.Rotation = (_hg.Rotation + spd) % 360
        end
    end)
end
...
do
    local _tgl = Instance.new("UIGradient")
    _tgl.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(0,   30,  130)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(30,  120, 255)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(140, 210, 255)),
    })
    _tgl.Rotation = 0
    _tgl.Parent   = hubTitleLbl
    task.spawn(function()
        local spd = 1.2
        while task.wait() do
            _tgl.Rotation = (_tgl.Rotation + spd) % 360
        end
    end)
end
...
-- Discord Billboard فوق الـ character
task.spawn(function()
    local DISCORD_LINK = "discord.gg/aCbdbWpch"
    ...
    local function addDiscordBillboard(char)
        ...
        -- شيل القديم لو موجود
        local old = hrp:FindFirstChild("VenomDiscordBB")
        if old then old:Destroy() end

        local bb = Instance.new("BillboardGui")
        bb.Name            = "VenomDiscordBB"
        ...
        bbGrad.Rotation = 0
        bbGrad.Parent   = bbStroke
        task.spawn(function()
            while bb and bb.Parent do
                task.wait()
                bbGrad.Rotation = (bbGrad.Rotation + 1.5) % 360
            end
        end)
        ...
        lbl.TextStrokeColor3     = Color3.fromRGB(0, 10, 60)

        -- gradient دوار على النص
        ...
        task.spawn(function()
            while bb and bb.Parent do
                task.wait()
                lblGrad.Rotation = (lblGrad.Rotation + 1.2) % 360
            end
        end)
    end
