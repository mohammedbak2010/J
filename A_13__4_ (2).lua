...
task.spawn(function()
    ...
    local function makePBtn(l1, l2, defaultX, defaultY)
        ...
        frame.Name = "BtnFrame"; frame.Size = UDim2.new(0,56,0,50)
        ...
        local bs = Instance.new("UIStroke", frame); bs.Color=Color3.fromRGB(40,40,50); bs.Thickness=1; bs.Transparency=0.2

        local t1 = Instance.new("TextLabel", frame); t1.Size=UDim2.new(1,0,0.52,0); t1.Position=UDim2.new(0,0,0.06,0); t1.BackgroundTransparency=1; t1.Text=l1; t1.TextColor3=Color3.fromRGB(255,255,255); t1.Font=Enum.Font.GothamBlack; t1.TextSize=10; t1.TextXAlignment=Enum.TextXAlignment.Center; t1.ZIndex=22
        local t2 = Instance.new("TextLabel", frame); t2.Size=UDim2.new(1,0,0.44,0); t2.Position=UDim2.new(0,0,0.52,0); t2.BackgroundTransparency=1; t2.Text=l2; t2.TextColor3=Color3.fromRGB(255,255,255); t2.Font=Enum.Font.GothamBlack; t2.TextSize=10; t2.TextXAlignment=Enum.TextXAlignment.Center; t2.ZIndex=22
        ...
        -- drag logic per button (H2N style: activeInputId + moveState table)
        local bDragging=false; local bDragStart=nil; local bFrameStart=nil
        local moveState={moved=false}
        ...
        frame.InputBegan:Connect(function(inp)
            ...
            bDragging=true; moveState.moved=false; activeInputId=inp
            bDragStart=Vector2.new(inp.Position.X,inp.Position.Y)
            bFrameStart=Vector2.new(frame.Position.X.Offset,frame.Position.Y.Offset)
        end)

        UserInputService.InputChanged:Connect(function(inp)
            if not bDragging or inp~=activeInputId then return end
            local t=inp.UserInputType
            if t~=Enum.UserInputType.MouseMovement and t~=Enum.UserInputType.Touch then return end
            if not bDragStart or not bFrameStart then return end
            local dx=inp.Position.X-bDragStart.X; local dy=inp.Position.Y-bDragStart.Y
            if math.abs(dx)>4 or math.abs(dy)>4 then
                moveState.moved=true
                frame.Position=UDim2.new(0,bFrameStart.X+dx,0,bFrameStart.Y+dy)
            end
        end)
        ...
        UserInputService.InputEnded:Connect(function(inp)
            ...
            local t=inp.UserInputType
            if t~=Enum.UserInputType.MouseButton1 and t~=Enum.UserInputType.Touch then return end
            bDragging=false; bDragStart=nil; bFrameStart=nil; activeInputId=nil
        end)

        return btn, bs, frame, moveState
    end
    ...
    -- Default positions: spread along right side
    local bW=56; local bH=50; local startX=math.floor(workspace.CurrentCamera.ViewportSize.X)-bW-8
    local sy=math.floor(workspace.CurrentCamera.ViewportSize.Y/2)-((bH+6)*4)/2

    local btnAAL,bsAAL,fAAL,msAAL = makePBtn("AUTO","L", startX, sy)
    local btnAAR,bsAAR,fAAR,msAAR = makePBtn("AUTO","R", startX, sy+(bH+6))
    local btnPL,bsPL,fPL,msPL     = makePBtn("PLAY","L", startX, sy+(bH+6)*2)
    local btnPR,bsPR,fPR,msPR     = makePBtn("PLAY","R", startX, sy+(bH+6)*3)
    local btnDR,bsDR,fDR,msDR     = makePBtn("DROP","", startX, sy+(bH+6)*4)
    local btnTP,bsTP,fTP,msTP     = makePBtn("TP","DOWN", startX, sy+(bH+6)*5)
    local btnLK,bsLK,fLK,msLK     = makePBtn("LOCK","", startX, sy+(bH+6)*6)
    local btnCS,bsCS,fCS,msCS     = makePBtn("CARRY","SPD", startX, sy+(bH+6)*7)
    ...
    btnAAL.MouseButton1Up:Connect(function() if msAAL.moved then msAAL.moved=false; return end; local ns=not pState.AAL; if ns then mutualOff("AAL") end; pState.AAL=ns; setPH(fAAL,bsAAL,ns); mobBtnRefs["AUTO L"].state=ns; if toggleSetters["Auto Left"] then toggleSetters["Auto Left"](ns) end; if ns then autoLOn=true; startAutoL() else autoLOn=false; stopAutoL() end end)
    btnAAR.MouseButton1Up:Connect(function() if msAAR.moved then msAAR.moved=false; return end; local ns=not pState.AAR; if ns then mutualOff("AAR") end; pState.AAR=ns; setPH(fAAR,bsAAR,ns); mobBtnRefs["AUTO R"].state=ns; if toggleSetters["Auto Right"] then toggleSetters["Auto Right"](ns) end; if ns then autoROn=true; startAutoR() else autoROn=false; stopAutoR() end end)
    btnPL.MouseButton1Up:Connect(function() if msPL.moved then msPL.moved=false; return end; local ns=not pState.PL; if ns then mutualOff("PL") end; pState.PL=ns; setPH(fPL,bsPL,ns); mobBtnRefs["PLAY L"].state=ns; if toggleSetters["Play Left"] then toggleSetters["Play Left"](ns) end; if ns then aplOn=true; startAutoPlayLeft() else aplOn=false; stopAutoPlayLeft() end end)
    btnPR.MouseButton1Up:Connect(function() if msPR.moved then msPR.moved=false; return end; local ns=not pState.PR; if ns then mutualOff("PR") end; pState.PR=ns; setPH(fPR,bsPR,ns); mobBtnRefs["PLAY R"].state=ns; if toggleSetters["Play Right"] then toggleSetters["Play Right"](ns) end; if ns then aprOn=true; startAutoPlayRight() else aprOn=false; stopAutoPlayRight() end end)
    btnDR.MouseButton1Up:Connect(function() if msDR.moved then msDR.moved=false; return end; setPH(fDR,bsDR,true); task.delay(0.35,function() setPH(fDR,bsDR,false) end); task.spawn(doDrop) end)
    btnTP.MouseButton1Up:Connect(function() if msTP.moved then msTP.moved=false; return end; setPH(fTP,bsTP,true); task.delay(0.35,function() setPH(fTP,bsTP,false) end); task.spawn(doTPDown) end)
    btnLK.MouseButton1Up:Connect(function() if msLK.moved then msLK.moved=false; return end; local ns=not pState.LK; if ns then mutualOff("LK") end; pState.LK=ns; setPH(fLK,bsLK,ns); if toggleSetters["Lock"] then toggleSetters["Lock"](ns) end; if ns then startBatAimbot() else stopBatAimbot() end end)
    btnCS.MouseButton1Up:Connect(function() if msCS.moved then msCS.moved=false; return end; local ns=not pState.CS; pState.CS=ns; setPH(fCS,bsCS,ns); slowDownEnabled=ns; if toggleSetters["Carry SPD"] then toggleSetters["Carry SPD"](ns) end end)
