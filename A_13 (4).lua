...
task.spawn(function()
    ...
    local function makePBtn(l1, l2, defaultX, defaultY)
        ...
        -- drag logic per button
        local bDragging=false; local bDragStart=nil; local bFrameStart=nil; local bMoved=false
        ...
        UserInputService.InputEnded:Connect(function(inp)
            if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
                bDragging=false; bDragStart=nil; bFrameStart=nil
            end
        end)
        UserInputService.InputChanged:Connect(function(inp)
            if not bDragging or not bDragStart then return end
            if inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch then
                local dx=inp.Position.X-bDragStart.X; local dy=inp.Position.Y-bDragStart.Y
                if math.abs(dx)>4 or math.abs(dy)>4 then
                    bMoved=true
                    frame.Position=UDim2.new(frame.Position.X.Scale,bFrameStart.X+dx,frame.Position.Y.Scale,bFrameStart.Y+dy)
                end
            end
        end)

        return btn, bs, frame, bMoved
    end
    ...
    local _b,bsAAL,fAAL,_ = makePBtn("AUTO","L", startX, sy)
    local btnAAL=_b
    local _b,bsAAR,fAAR,_ = makePBtn("AUTO","R", startX, sy+(bH+6))
    local btnAAR=_b
    local _b,bsPL,fPL,_ = makePBtn("PLAY","L", startX, sy+(bH+6)*2)
    local btnPL=_b
    local _b,bsPR,fPR,_ = makePBtn("PLAY","R", startX, sy+(bH+6)*3)
    local btnPR=_b
    local _b,bsDR,fDR,_ = makePBtn("DROP","", startX, sy+(bH+6)*4)
    local btnDR=_b
    local _b,bsTP,fTP,_ = makePBtn("TP","DOWN", startX, sy+(bH+6)*5)
    local btnTP=_b
    local _b,bsLK,fLK,_ = makePBtn("LOCK","", startX, sy+(bH+6)*6)
    local btnLK=_b
    local _b,bsCS,fCS,_ = makePBtn("CARRY","SPD", startX, sy+(bH+6)*7)
    local btnCS=_b
    ...
    btnAAL.MouseButton1Click:Connect(function() local ns=not pState.AAL; if ns then mutualOff("AAL") end; pState.AAL=ns; setPH(fAAL,bsAAL,ns); mobBtnRefs["AUTO L"].state=ns; if toggleSetters["Auto Left"] then toggleSetters["Auto Left"](ns) end; if ns then autoLOn=true; startAutoL() else autoLOn=false; stopAutoL() end end)
    btnAAR.MouseButton1Click:Connect(function() local ns=not pState.AAR; if ns then mutualOff("AAR") end; pState.AAR=ns; setPH(fAAR,bsAAR,ns); mobBtnRefs["AUTO R"].state=ns; if toggleSetters["Auto Right"] then toggleSetters["Auto Right"](ns) end; if ns then autoROn=true; startAutoR() else autoROn=false; stopAutoR() end end)
    btnPL.MouseButton1Click:Connect(function() local ns=not pState.PL; if ns then mutualOff("PL") end; pState.PL=ns; setPH(fPL,bsPL,ns); mobBtnRefs["PLAY L"].state=ns; if toggleSetters["Play Left"] then toggleSetters["Play Left"](ns) end; if ns then aplOn=true; startAutoPlayLeft() else aplOn=false; stopAutoPlayLeft() end end)
    btnPR.MouseButton1Click:Connect(function() local ns=not pState.PR; if ns then mutualOff("PR") end; pState.PR=ns; setPH(fPR,bsPR,ns); mobBtnRefs["PLAY R"].state=ns; if toggleSetters["Play Right"] then toggleSetters["Play Right"](ns) end; if ns then aprOn=true; startAutoPlayRight() else aprOn=false; stopAutoPlayRight() end end)
    btnDR.MouseButton1Click:Connect(function() setPH(fDR,bsDR,true); task.delay(0.35,function() setPH(fDR,bsDR,false) end); task.spawn(doDrop) end)
    btnTP.MouseButton1Click:Connect(function() setPH(fTP,bsTP,true); task.delay(0.35,function() setPH(fTP,bsTP,false) end); task.spawn(doTPDown) end)
    btnLK.MouseButton1Click:Connect(function() local ns=not pState.LK; if ns then mutualOff("LK") end; pState.LK=ns; setPH(fLK,bsLK,ns); if toggleSetters["Lock"] then toggleSetters["Lock"](ns) end; if ns then startBatAimbot() else stopBatAimbot() end end)
    btnCS.MouseButton1Click:Connect(function() local ns=not pState.CS; pState.CS=ns; setPH(fCS,bsCS,ns); slowDownEnabled=ns; if toggleSetters["Carry SPD"] then toggleSetters["Carry SPD"](ns) end end)
