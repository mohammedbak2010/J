...
local GLOBAL_CONFIG_FILE = "A_13_Config.json"
local _globalConfig = {}
...
local _DEFAULT_CONFIG = {
    ...
    togglesState    = {
        speedCustomizer=false, spin=true,    noAnim=false,
...
pcall(function()
    if CoreGui:FindFirstChild("A_13") then
        CoreGui:FindFirstChild("A_13"):Destroy()
    end
end);
...
-- Discord Billboard
task.spawn(function()
    local function addDiscordBillboard(char)
        local head = char:WaitForChild("Head", 5)
        if not head then return end
        local old = char:FindFirstChild("DiscordBB")
    ...
    if lp.Character then
        task.spawn(addDiscordBillboard, lp.Character)
    end
    lp.CharacterAdded:Connect(function(char)
        task.wait(0.3)
        addDiscordBillboard(char)
    end)
end)
...
local C = {
    Black     = Color3.fromRGB(5, 12, 20),
    DarkRow   = Color3.fromRGB(10, 20, 38),
    DarkTab   = Color3.fromRGB(6, 14, 28),
    Gray      = Color3.fromRGB(100, 200, 240),
    LightGray = Color3.fromRGB(80, 170, 220),
    White     = Color3.fromRGB(200, 240, 255),
    Dim       = Color3.fromRGB(40, 120, 180),
    TogOff    = Color3.fromRGB(10, 30, 55),
}

local STROKE_ACTIVE = Color3.fromRGB(0, 180, 255)
local STROKE_IDLE   = Color3.fromRGB(5, 20, 40)
local DOT_ON  = Color3.fromRGB(100, 220, 255)
local DOT_OFF = Color3.fromRGB(20, 60, 100)
...
local function applyHubGradient(frame)
    frame.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
    frame.BackgroundTransparency = 0
    local grad = Instance.new("UIGradient", frame)
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(12,  5,  30)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(8,   3,  20)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(18, 10,  40)),
    })
    grad.Rotation = 135
end
...
do
    ...
    tpDownFrame.BackgroundColor3 = Color3.fromRGB(8,5,18)
    ...
    tpDownStroke.Thickness = 2; tpDownStroke.Color = Color3.fromRGB(160, 50, 255)
    tpDownStroke.Transparency = 0
    do
        local _tds = Instance.new("UIGradient", tpDownStroke)
        _tds.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0,   Color3.fromRGB(80,   0,  180)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160,  40, 255)),
            ColorSequenceKeypoint.new(1,   Color3.fromRGB(220, 140, 255)),
        })
        _tds.Rotation = 0
        task.spawn(function()
            while task.wait() do _tds.Rotation = (_tds.Rotation + 1.5) % 360 end
        end)
    end
...
miniFrame.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
...
mStroke.Thickness    = 2
mStroke.Color        = Color3.fromRGB(140, 50, 255)
mStroke.Transparency = 0
applyHubGradient(miniFrame)
do
    local _mg = Instance.new("UIGradient", mStroke)
    ...
    _mg.Rotation = 0
    task.spawn(function()
        while task.wait() do _mg.Rotation = (_mg.Rotation + 1.5) % 360 end
    end)
end
...
do
    local _tg = Instance.new("UIGradient", menuBtn)
    ...
    _tg.Rotation = 0
    task.spawn(function()
        while task.wait() do _tg.Rotation = (_tg.Rotation + 1.2) % 360 end
    end)
end
...
MainFrame.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
...
hStroke.Thickness    = 2
hStroke.Color        = Color3.fromRGB(140, 50, 255)
hStroke.Transparency = 0
do
    local _hg = Instance.new("UIGradient", hStroke)
    _hg.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(80,   0,  180)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160,  40, 255)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(220, 140, 255)),
    })
    _hg.Rotation = 0
    task.spawn(function()
        while task.wait() do _hg.Rotation = (_hg.Rotation + 1.5) % 360 end
    end)
end
...
local hubTitleLbl = New("TextLabel", {
    Text = "VENOM DUELS", TextColor3 = Color3.fromRGB(100, 210, 255),
...
do
    local _titleGrad = Instance.new("UIGradient", hubTitleLbl)
    ...
    _titleGrad.Rotation = 0
    task.spawn(function()
        while task.wait(0.02) do _titleGrad.Rotation = (_titleGrad.Rotation + 2) % 360 end
    end)
end
...
do
    local _tcStroke = Instance.new("UIStroke", TabContainer)
    _tcStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    _tcStroke.Thickness = 1.2
    _tcStroke.Color = Color3.fromRGB(140, 50, 255)
    _tcStroke.Transparency = 0
    local _tcg = Instance.new("UIGradient", _tcStroke)
    _tcg.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(80,   0,  180)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160,  40, 255)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(220, 140, 255)),
    })
    _tcg.Rotation = 0
    task.spawn(function()
        while task.wait() do _tcg.Rotation = (_tcg.Rotation + 1.5) % 360 end
    end)
end
...
local function Toggle(page, label, default, cb)
    ...
    do
        ...
        _rs.Thickness = 1.2
        _rs.Color = Color3.fromRGB(140, 50, 255)
        _rs.Transparency = 0
        local _rg = Instance.new("UIGradient", _rs)
        _rg.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0,   Color3.fromRGB(80,   0,  180)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160,  40, 255)),
            ColorSequenceKeypoint.new(1,   Color3.fromRGB(220, 140, 255)),
        })
        _rg.Rotation = 0
        task.spawn(function()
            while row and row.Parent do task.wait(); _rg.Rotation = (_rg.Rotation + 1.5) % 360 end
        end)
    end
    New("TextLabel", {
    ...
    local bg = New("Frame", {
        ...
        BackgroundColor3=on and Color3.fromRGB(140,50,255) or C.TogOff, ZIndex=8, Parent=row,
    })
    ...
    New("TextButton", {
        ...
        on = not on
        TweenService:Create(bg,  TweenInfo.new(0.18), {BackgroundColor3=on and Color3.fromRGB(140,50,255) or C.TogOff}):Play()
...
topFrame.Position = UDim2.new(0.5, -95, 0, 10)
topFrame.AnchorPoint = Vector2.new(0, 0)
topFrame.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
...
topStroke.Color = Color3.fromRGB(160, 50, 255)
topStroke.Transparency = 0
do
    local _tsg = Instance.new("UIGradient", topStroke)
    _tsg.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(80,   0,  180)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160,  40, 255)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(220, 140, 255)),
    })
    _tsg.Rotation = 0
    task.spawn(function()
        while task.wait() do _tsg.Rotation = (_tsg.Rotation + 1.5) % 360 end
    end)
end
do
...
botFrame.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
...
botStroke.Color = Color3.fromRGB(160, 50, 255)
botStroke.Transparency = 0
do
    local _bsg = Instance.new("UIGradient", botStroke)
    _bsg.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(80,   0,  180)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160,  40, 255)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(220, 140, 255)),
    })
    _bsg.Rotation = 0
    task.spawn(function()
        while task.wait() do _bsg.Rotation = (_bsg.Rotation + 1.5) % 360 end
    end)
end
gbStroke = botStroke
do
    ...
    grabBarFill.BackgroundColor3 = Color3.fromRGB(160, 50, 255)
...
local function createDuelHub()
    ...
    local function setAutoTPVisual(on) autoTPAtivo = on end

    btnPR.MouseButton1Click:Connect(function() end);
    btnPL.MouseButton1Click:Connect(function() end);
...
(function()
    ...
    tFrame.BackgroundColor3 = Color3.fromRGB(8,5,18)
...
LockFrame.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
...
ToggleSaved("Combat", "Auto TP", "hitTP", true, function(v)
...
do
    ...
    if getgenv then
        if getgenv().VENOM_AUTOLOAD == nil then
            getgenv().VENOM_AUTOLOAD = false
        end
    end

    local autoLoadEnabled = (getgenv and getgenv().VENOM_AUTOLOAD == true) or false
    local kickConn = nil

    if getgenv then
        if not getgenv().VENOM_LAST_RELOAD then
            getgenv().VENOM_LAST_RELOAD = 0
        end
    end
    ...
    local function doReload()
        ...
        local now = tick()
        if getgenv and (now - (getgenv().VENOM_LAST_RELOAD or 0)) < 5 then return end
        if getgenv then getgenv().VENOM_LAST_RELOAD = now end
    ...
    local function setAutoLoad(v)
        ...
        if getgenv then getgenv().VENOM_AUTOLOAD = v end
